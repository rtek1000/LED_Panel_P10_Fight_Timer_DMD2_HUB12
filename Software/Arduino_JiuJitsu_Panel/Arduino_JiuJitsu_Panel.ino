...
void controller(void) {
  ...
  if (((digitalRead(input_ctrl_left_pin) == false) && (digitalRead(input_ctrl_cent_pin) == false)) ||
      ((digitalRead(input_ctrl_left_pin) == false) && (digitalRead(input_ctrl_right_pin) == false)) ||
      ((digitalRead(input_ctrl_cent_pin) == false) && (digitalRead(input_ctrl_right_pin) == false))) {
    return;
  }

  if ((digitalRead(input_ctrl_left_pin) == false) && (input_ctrl_left_pin_old == true)) // HIGH
  {
    input_ctrl_left_pin_old = false;

    input_ctrl_left = true;
  } else if (digitalRead(input_ctrl_left_pin) == true) {
    input_ctrl_left_pin_old = true;
  }

  if ((digitalRead(input_ctrl_cent_pin) == false) && (input_ctrl_cent_pin_old == true)) // HIGH
  {
    input_ctrl_cent_pin_old = false;

    input_ctrl_cent = true;
  } else if (digitalRead(input_ctrl_cent_pin) == true) {
    input_ctrl_cent_pin_old = true;
  }

  if ((digitalRead(input_ctrl_right_pin) == false) && (input_ctrl_right_pin_old == true)) // HIGH
  {
    input_ctrl_right_pin_old = false;

    input_ctrl_right = true;
  } else if (digitalRead(input_ctrl_right_pin) == true) {
...
void show_date(void) {
  ...
  wr_rtc_HH = hour(t); //now.hour();
  wr_rtc_MM = minute(t); //now.minute();

  wr_rtc_yy = year(t); //now.year();
  wr_rtc_mm = month(t); //now.month();
  wr_rtc_dd = day(t); // now.day();
  ...
  dmd.drawString(2, line2, daysOfTheWeek[weekday(t) - 1]); // now.dayOfTheWeek()]);
}
